...
// GET /api/client/products — catálogo disponível para clientes logados.
router.get('/products', async (req, res) => {
  try {
    const products = await Product.find({ inStock: true })
      .select('name priceCents imageUrl category inStock')
...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const history = await Redemption.find({ userId: user._id, deletedAt: { $exists: false } }).sort({ createdAt: -1 }).limit(10);
...
router.get('/history', async (req, res) => {
  try {
    const history = await Redemption.find({ userId: req.user.id, deletedAt: { $exists: false } }).sort({ createdAt: -1 });
    res.json({ history });
  } catch (err) {
